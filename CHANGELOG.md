...
- apps.nix: bake Wayland screen-share flags into a `vesktop` binary wrapper (runCommand shipping bin + package icons) instead of the desktop entry — walker/elephant-launched Vesktop lost streaming flags / black-screened; wrapper guarantees --ozone-platform-hint=auto --enable-features=WebRTCPipeWireCapturer --disable-gpu-sandbox from any launcher
